...
export function plainFromSections(sections: TemplateSection[]): string {
  return sections
    .map((sec) => {
      const parts: string[] = [];
      if (sec.heading && sec.heading.trim()) parts.push(HEADING_PREFIX + sec.heading.trim());
...
export function rtfFromSections(sections: TemplateSection[]): string {
  const parts: string[] = [
