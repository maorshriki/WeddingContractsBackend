...
import { rowToContract } from '../types/contract';
...
router.get('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const contracts = rows.map(rowToContract);
    res.json(contracts);
  } catch (err) {
...
router.post('/', async (req: Request, res: Response) => {
  ...
  const eventDateStr = ed.eventDate ? (ed.eventDate as string).slice(0, 10) : new Date().toISOString().slice(0, 10);
  const startTimeStr = ed.startTime ? (typeof ed.startTime === 'string' && ed.startTime.includes('T') ? (ed.startTime as string).slice(11, 16) : (ed.startTime as string)) : '19:00';
  try {
    const { rows } = await pool.query(
